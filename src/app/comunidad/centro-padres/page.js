...
export default function page() {
  ...
  const selectedPersonaje = (index) => {
    setActive(centroPadres[index]);
  };
  if (!centroPadres || centroPadres.length === 0) {
    return <div>Loading...</div>; // O cualquier mensaje de carga que prefieras
  }
  return (
    <div className="max-w-screen-xl min-h-screen pt-16 xl:pt-24 content-center">
      <div className="grid xl:grid-cols-2 gap-4">
        ...
        <div className="xl:col-span-2 order-0 xl:order-1">
          <div className={`gap-1 md:gap-4 flex justify-center`}>
            {centroPadres.map((personaje, index) => (
              <Image
                key={index}
